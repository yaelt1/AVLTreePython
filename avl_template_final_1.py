...
class AVLTreeList(object):
    ...
    def find_max(self, node):  # O(log(n))
        cur_node = node
    ...
    def update_tree(self, node):  # O(log(n))
    ...
    # only for insert
    def rebalance(self, node):  # O(log(n))
        count = 0
        if self.size > 2:
    ...
    def rotate_right(self, node):  # O(1)
        son = node.left
    ...
    def find_min(self, node):  # O(log(n))
        cur_node = node
    ...
    def rebalance_delete(self, node):  # O(log(n))
        count = 0
        if self.size > 2:
            while node is not None:
                self.update_node(node)
    ...
    def merge_sort(self, lst):  # O(nlog(n))
        len_lst = len(lst)
        if len_lst <= 1:
            return lst
        else:
    ...
    def virtual_sons(self, node):  # O(1)
        # left virtual son
        v1 = AVLNode(None)
        v1.setHeight(-1)
        v1.size = 0
